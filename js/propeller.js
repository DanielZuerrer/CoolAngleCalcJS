...
(function (w) {
    ...
    p.bindHandlers = function () {
        this.onRotationStart = this
            .onRotationStart
            .bind(this);
        this.onRotationStop = this
            .onRotationStop
            .bind(this);
        this.onRotated = this
            .onRotated
    ...
    p.updateAngleToMouse = function (event) {
        ...
        // At this moment we have to use specific algorithm when CSS transition is
        // enabled. Using same approach when transition is disabled lead to worse
        // precision.
        // TODO Develop universal algorithm to support transitions and provide high
        // precision at once
    ...
    p.initOptions = function (options) {
        ...
        this.minimalAngleChange = this.step !== defaults.step
            ? this.step
            : defaults.minimalAngleChange;
    ...
    p.initHardwareAcceleration = function () {
        ...
        // If CSS3d is supported then ann translateZ hack to enable GPU acceleration on
        // layer
        if (supported === true) {
    ...
    p.getViewOffset = function (singleFrame) {
        ...
        if (this.element)
            this.addOffset(this.element, coords, 'defaultView' in document
    ...
    p.addOffset = function (node, coords, view) {
        ...
        coords.x += node.offsetLeft - (p
            ? p.scrollLeft
            : 0);
        coords.y += node.offsetTop - (p
            ? p.scrollTop
            : 0);
